...
public class HexGrid : MonoBehaviour
{
	...
	public HexUnit fighterPrefab;
	public HexUnit golemPrefab;
	...
	public int PathDistance => searchData[currentPathToIndex].distance;
    public HexCell PathToLocation => GetCell(currentPathToIndex);
...
#pragma warning restore IDE0044 // Add readonly modifier
    ...
    void Update()
    {
		HexCell hover_cell = GetCell(Camera.main.ScreenPointToRay(Input.mousePosition));
		if (hover_cell && timer <= 0)
        {
			timer = 2f;
			//Debug.Log("Cursor over cell: " + hover_cell.Coordinates.X + ", " + hover_cell.Coordinates.Z);
        }
  ...
  //      TextMeshProUGUI data = Instantiate(cellDataPrefab, cell.Position + (Vector3.up / 2), Quaternion.identity).GetComponentInChildren<TextMeshProUGUI>();
		//data.text = $"{cell.Coordinates.X}, {cell.Coordinates.Z}";
	...
	public void Save(BinaryWriter writer)
	{
		...
		for (int i = 0; i < CellData.Length; i++)
		{
            ...
            data.valuesExtended.Save(writer);
	...
	public void Load(BinaryReader reader, int header)
	{
		...
		for (int i = 0; i < CellData.Length; i++)
		{
			...
			data.valuesExtended = HexValuesExtended.Load(reader, header);
		...
		if (header >= 2)
		{
			...
			{
				HexUnit.Load(reader, this, unitManager);
			}
		}
	...
	public int FindPath(HexCell fromCell, HexCell toCell, HexUnit unit)
	{
		ClearPath();
		currentPathFromIndex = fromCell.Index;
        currentPathToIndex = Search(fromCell, toCell, unit);
		currentPathExists = currentPathToIndex == -1 ? false : true;
		if (!currentPathExists)
		{
            currentPathToIndex = toCell.Index;
        }
		ShowPath();
		return searchData[currentPathToIndex].distance;
	}

	int Search(HexCell fromCell, HexCell toCell, HexUnit unit)
	{
		...
		while (searchFrontier.TryDequeue(out int currentIndex))
		{
			...
			for (HexDirection d = HexDirection.NE; d <= HexDirection.NW; d++)
			{
                ...
                if (lastIndex == -1)
                {
                    lastIndex = neighbor.Index;
                }
                else if (CellData[lastIndex].coordinates.DistanceTo(toCell.Coordinates) >
	...
	{
		List<string> cell_json = new List<string>();
		for (int i=0; i<CellData.Length; i++)
        {
			if (CellData[i].HasResource)
			{
	            cell_json.Add(CellData[i].ToJSON());
			}
        }
        string _json = "\"resources\": [" + string.Join(",", cell_json) + "]";
        return _json;
    }
}
