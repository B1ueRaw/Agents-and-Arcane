	...
	void SetTerrainType()
	{
		...
		for (int i = 0; i < cellCount; i++)
		{
			...
			if (!cell.IsUnderwater)
			{
				...
				mana += cellBiome.plant;
				terrain = cellBiome.terrain;

				grid.CellData[i].values = cell.values.
					WithTerrainTypeIndex(cellBiome.terrain).
					WithPlantLevel(cellBiome.plant);
				grid.CellData[i].valuesExtended = cell.valuesExtended.
					WithResourceLevel(20).
					WithResourceTypeIndex(1);
			}
			else
			{
			...
			{
				case 0:
					cell.valuesExtended = cell.valuesExtended.WithResourceTypeIndex(1);
					//Debug.Log((int)Resources.COPPER + ", " + cell.values.ResourceTypeIndex);
					break;
				case 1:
                    cell.valuesExtended = cell.valuesExtended.WithResourceTypeIndex(4);
					mana += 10;
					break;
				case 2:
                    cell.valuesExtended = cell.valuesExtended.WithResourceTypeIndex(1);
					break;
				case 3:
                    cell.valuesExtended = cell.valuesExtended.WithResourceTypeIndex(2);
					break;
				case 4:
                    cell.valuesExtended = cell.valuesExtended.WithResourceTypeIndex(3);
					break;
				default:
                    cell.valuesExtended = cell.valuesExtended.WithResourceTypeIndex(2);
					break;
            ...
            // Set mana and resource types
            if (mana < 0) mana = 0;
            if (mana > 31) mana = 31;
            cell.valuesExtended = cell.valuesExtended.WithManaLevel(mana);
            Debug.Log(cell.valuesExtended.ResourceTypeIndex);

			if (cell.IsUnderwater) 
			{
                grid.CellData[i].values = cell.values.WithTerrainTypeIndex(terrain);
			}
			else if (Random.Range(1, 100) <= resourcePercentage)
			{
                cell.valuesExtended = cell.valuesExtended.WithResourceLevel(Random.Range(1, 31));
			}
			else
			{
                cell.valuesExtended = cell.valuesExtended.WithResourceLevel(0);
			}
			grid.CellData[i].valuesExtended = cell.valuesExtended;
        }
	}
