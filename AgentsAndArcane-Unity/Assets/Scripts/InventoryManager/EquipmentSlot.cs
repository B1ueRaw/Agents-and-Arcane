...
public class EquipmentSlot : MonoBehaviour, IPointerClickHandler, IDataPersistence
{
    //======ITEM DATA======//
    ...
    public void OnLeftClick()
    {
        if (isSelected)
        {
            // check if the item match the unit type
            if (equipment == null) return;
            if (hexUnit.itemBelongsTo == equipment.itemBelongsTo)
            {
                if (this.quantity == 1) {
                    EquipGear(this.itemSprite, this.equipment, this.hexUnit);
                    emptySlot();
                } else {
                    this.quantity -= 1;
                    quantityText.text = this.quantity.ToString();
    ...
    private void EquipGear(Sprite itemSprite, Equipment equipment, HexUnit hexUnit)
    {
        // TODO: Add check unit types function to check if the item can be equipped by the unit
        equippedSlot.EquipGear(itemSprite, this.equipment, hexUnit);
    }
